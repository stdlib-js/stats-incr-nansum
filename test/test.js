...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof incrnansum, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrnansum(), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'if not provided an input value, the accumulator function returns the current sum', function test( t ) {
	...
	t.strictEqual( acc(), 6.0, 'returns the current accumulated sum' );
	t.end();
});
